...
async function seedDatabase() {
  ...
  for (const user of users) {
    try {
      ...
      // Generate QR code
      const qrCode = await QRCode.toDataURL(\https://fulfilling-commitment-production.up.railway.app/visitor?user=\\)

      // Insert user
      const stmt = db.prepare(\
        INSERT OR IGNORE INTO users (username, password, familia, direccion, email, localidad, codigo_postal, qr_code)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      \)
